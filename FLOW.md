...
| **MD5 Cache** | hash(prompt_content) → fișier JSON pe disc | Re-rulări gratuite în development și reluare state (Resume) pe cluster (FEP) |
| **SLURM Job Chaining** | `--chain N` argument pentru a segmenta fișierele lungi | Suport pentru documente imense în medii cu limită dură de timp pentru executare (ex. 10min/job FEP) |
