...
def _archive_run(state: KnowledgeStateTracker, runs_dir: str = None):
    """Save a timestamped copy of the run to <data_dir>/runs/."""
    if runs_dir is None:
        runs_dir = os.path.join(settings.DATA_DIR, "runs")
...
def main(fresh_run=False, blueprint_only=False, input_file=None):
    print("=" * 60)
    print("  DECOMPOSED AXIOM-BY-AXIOM (AbA) ONTOLOGY PIPELINE")
    print("=" * 60)

    # Derive per-run directories from DATA_DIR (can be overridden via --data-dir or DATA_DIR env)
    data_dir = settings.DATA_DIR
    output_dir = os.path.join(data_dir, "output")
    cache_dir = os.path.join(data_dir, "cache")
    chunks_dir = os.path.join(data_dir, "chunks")
    runs_dir = os.path.join(data_dir, "runs")
    rate_limit_file = os.path.join(data_dir, "rate_limit_state.json")
    ...
    if input_file:
        doc_path = os.path.join(settings.INPUT_DIR, input_file)
    ...
    # ── Save Blueprint ──
    blueprint_path = os.path.join(output_dir, "ontology_blueprint.json")
    pure_schema = {
        "approved_classes": state.approved_classes,
        "approved_properties": state.approved_properties,
        "canonical_terms": state.canonical_terms,
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            ...
            state.save_state(os.path.join(output_dir, "ontology_state_partial.json"))
            state.export_turtle(os.path.join(output_dir, "ontology_partial.ttl"))
            _archive_run(state, runs_dir)
    ...
    # Save final state
    state.save_state(os.path.join(output_dir, "ontology_state.json"))
    state.export_turtle(os.path.join(output_dir, "ontology_final.ttl"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-dir",
        default=None,
        help="Isolated data directory for this run (overrides DATA_DIR env var). "
             "Use for parallel runs so each has its own cache, output and runs.",
    )
    parser.add_argument(
        "--input",
        type=str,
        default=None,
        help="Specific input file inside data/input (e.g. my_doc.pdf)",
    )
    args = parser.parse_args()

    # Apply --data-dir before any component reads settings.DATA_DIR
    if args.data_dir:
    ...
    if args.reset_rate_limit:
        rl = RateLimiter(
            ...
            max_rpd=settings.RATE_LIMIT_RPD,
            min_delay=settings.MIN_DELAY_BETWEEN_REQUESTS,
            state_file=os.path.join(settings.DATA_DIR, "rate_limit_state.json"),
        )
        rl.reset(remove_file=True)
        print("[MAIN] Rate limit state reset. Continuing with run.")

    main(fresh_run=args.fresh, blueprint_only=args.blueprint_only, input_file=args.input)
